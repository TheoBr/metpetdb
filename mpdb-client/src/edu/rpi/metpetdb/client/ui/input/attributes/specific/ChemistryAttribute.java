...
public class ChemistryAttribute extends GenericAttribute implements
	...
	public Widget[] createDisplayWidget(final MObjectDTO obj) {
		...
		// final Set s = get(obj); if (s != null) { final Iterator iter =
		// s.iterator(); while (iter.hasNext()) { final Object object =
		// iter.next(); final Label r = new Label(object.toString());
		// list.add(r); } }
		...
		if (species.getSelectedIndex() != -1 && !(species.isItemSelected(1))
				&& type.getSelectedIndex() != -1 && !(type.isItemSelected(1))) {

			final ListBox element_oxide = new ListBox();

			if (species.getSelectedIndex() == 2) {
				element_oxide.addItem("Element...", "Element...");
				element_oxide.addItem("Blah1...", "Blah1...");
			} else {
				element_oxide.addItem("Oxide...", "Oxide...");
				element_oxide.addItem("Blah2...", "Blah2...");
			}
		}
		...
		final Button adder = new Button("Add", new ClickListener() {
			public void onClick(final Widget sender) {
				if (species.getSelectedIndex() != -1
						&& !(species.isItemSelected(1))) {
					if (type.getSelectedIndex() != -1
							&& !(type.isItemSelected(1))) {
	...
	public Widget[] createEditWidget(final MObjectDTO obj, final String id) {
		...
		if (s != null) {
			...
			while (iter.hasNext()) {
				...
				editList.add(ChemistryAttribute.this
