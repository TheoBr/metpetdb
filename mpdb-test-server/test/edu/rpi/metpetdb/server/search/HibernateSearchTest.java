...
/*
	...
	public void testTermSearch() {
		...
		org.hibernate.Query hibQuery = fullTextSession.createFullTextQuery( termQuery, Sample.class );
	...
	public void testRangeSearch() {
		...
		RangeQuery rangeQuery = new RangeQuery(new Term("sesarNumber","000000004"), new Term("sesarNumber","000000009"),true);
		org.hibernate.Query hibQuery = fullTextSession.createFullTextQuery( rangeQuery, Sample.class );
		List<Sample> result = hibQuery.list();
		
		for(Sample s: result) {
			System.out.println("found sample, sesar number is " + s.getSesarNumber());
		}
	...
	public void testMultiFields() {
		...
		try
		{
			Query query =  org.apache.lucene.queryParser.MultiFieldQueryParser.parse(searchFor, columnsIn , flags, new StandardAnalyzer());
	...
	public void testJoin() {
		...
		try
		{
			...
			for(Sample s: result) {
				System.out.println("found sample, sesar number is " + s.getSesarNumber() + " username is " + s.getOwner().getUsername());
			}			
		}
		catch(ParseException e)
		{}
