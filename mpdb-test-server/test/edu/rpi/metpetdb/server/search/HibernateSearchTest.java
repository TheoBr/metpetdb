...
public class HibernateSearchTest extends DatabaseTestCase {
	...
	/*
	 * @Test public void testTermSearch() { final Session session =
	 * InitDatabase.getSession(); final FullTextSession fullTextSession = Search
	 * .createFullTextSession(session);
	 * 
	 * final TermQuery termQuery = new TermQuery(new Term("alias", "1")); final
	 * org.hibernate.Query hibQuery = fullTextSession
	 * .createFullTextQuery(termQuery, Sample.class); final List<Sample> result
	 * = hibQuery.list();
	 * 
	 ...
	 * 
	 * @Test public void testTermSearch2() { final Session session =
	 * InitDatabase.getSession(); final FullTextSession fullTextSession = Search
	 * .createFullTextSession(session);
	 * 
	 ...
	 * }
	 * 
	 * @Test public void testRangeSearch() { final Session session =
	 * InitDatabase.getSession(); final FullTextSession fullTextSession = Search
	 * .createFullTextSession(session);
	 * 
	 ...
	 * "000000004"), new Term("sesarNumber", "000000009"), true); final
	 * org.hibernate.Query hibQuery = fullTextSession
	 * .createFullTextQuery(rangeQuery, Sample.class); final List<Sample> result
	 * = hibQuery.list();
	 * 
	 * for (final Sample s : result)
	 ...
	 * tx.commit(); }
	 * 
	 * @Test public void testMultiFields() { final Session session =
	 * InitDatabase.getSession(); final FullTextSession fullTextSession = Search
	 * .createFullTextSession(session);
	 * 
	 * final Transaction tx = fullTextSession.beginTransaction();
	 * 
	 * System.out.println("in multifield search"); final String[] searchFor = {
	 * "000000004", "rockie rock" }; final String[] columnsIn = { "sesarNumber",
	 * "rockType" }; final BooleanClause.Occur[] flags = {
	 * BooleanClause.Occur.MUST, BooleanClause.Occur.MUST };
	 * 
	 * try { final Query query =
	 * org.apache.lucene.queryParser.MultiFieldQueryParser .parse(searchFor,
	 * columnsIn, flags, new StandardAnalyzer()); final org.hibernate.Query
	 * hibQuery = fullTextSession .createFullTextQuery(query, Sample.class);
	 * final List<Sample> result = hibQuery.list();
	 * 
	 * for (final Sample s : result)
	 * System.out.println("found sample, sesar number is " +
	 * s.getSesarNumber()); } catch (final ParseException e) { }
	 * 
	 * tx.commit(); }
	 * 
	 * @Test public void testJoin() { final Session session =
	 * InitDatabase.getSession(); final FullTextSession fullTextSession = Search
	 * .createFullTextSession(session);
	 * 
	 * final Transaction tx = fullTextSession.beginTransaction();
	 * 
	 * System.out.println("in test join"); final String[] searchFor = {
	 * "000000004", "anthony" }; final String[] columnsIn = { "sesarNumber",
	 * "user_username" }; final BooleanClause.Occur[] flags = {
	 * BooleanClause.Occur.MUST, BooleanClause.Occur.MUST };// ,
	 ...
	 * tx.commit(); }
	 * 
	 * @Test public void testSearchSampleSearch() { final SearchSample
	 * searchSamp = new SearchSample();
	 * 
	 * // searchSamp.setSesarNumber("000000000"); searchSamp.setAlias("1");
	 * final RockType rockType1 = new RockType();
	 * rockType1.setRockType("logitech"); final RockType rockType2 = new
	 * RockType(); rockType2.setRockType("rockie rock");
	 ...
	 * final Transaction tx = fullTextSession.beginTransaction();
	 * 
	 * final List<String> searchForValue = new LinkedList<String>(); final
	 * List<String> columnsIn = new LinkedList<String>(); final
	 * List<BooleanClause.Occur> flags = new LinkedList<BooleanClause.Occur>();
	 * 
	 * String columnName; Object methodResult = null; SearchProperty[] enums =
	 * SearchSampleProperty.class.getEnumConstants(); for (SearchProperty i :
	 * enums) { columnName = i.columnName(); methodResult = i.get(searchSamp);
	 * if (methodResult == null) { } else { if (methodResult instanceof Set) {
	 * for (Object o : (Set) methodResult) {
	 * System.out.println("adding a should for variable " + columnName +
	 * " with value " + o.toString()); searchForValue.add(o.toString());
	 * columnsIn.add(columnName); flags.add(BooleanClause.Occur.SHOULD); } }
	 * else { if (columnName.equals("publicData")) { } else {
	 * System.out.println("adding a must for variable " + columnName +
	 * " with value " + methodResult.toString());
	 * searchForValue.add(methodResult.toString()); columnsIn.add(columnName);
	 * flags.add(BooleanClause.Occur.MUST); } } } }
	 * 
	 * final String searchArray[] = new String[searchForValue.size()];
	 * searchForValue.toArray(searchArray); final String columnsArray[] = new
	 * String[columnsIn.size()]; columnsIn.toArray(columnsArray); final
	 * BooleanClause.Occur flagsArray[] = new BooleanClause.Occur[flags
	 * .size()]; flags.toArray(flagsArray); try { final Query query =
	 * org.apache.lucene.queryParser.MultiFieldQueryParser .parse(searchArray,
	 * columnsArray, flagsArray, new StandardAnalyzer()); final
	 * org.hibernate.Query hibQuery = fullTextSession
	 * .createFullTextQuery(query, Sample.class); final List<Sample> result =
	 * hibQuery.list(); assertEquals(1, result.size());
	 * System.out.println(query.toString()); for (final Sample s : result)
	 * System.out.println("found sample, sesar number is " +
	 * s.getSesarNumber()); } catch (final ParseException e) { }
	 * 
	 * tx.commit(); }
	 * 
	 * @Test public void testFindPublicSamples() { final Session session =
	 * InitDatabase.getSession(); final FullTextSession fullTextSession = Search
	 * .createFullTextSession(session);
	 * 
	 * final TermQuery termQuery = new TermQuery(new Term("publicData",
	 * Boolean.TRUE.toString())); final org.hibernate.Query hibQuery =
	 * fullTextSession .createFullTextQuery(termQuery, Sample.class); final
	 * List<Sample> results = hibQuery.list(); assertEquals(3, results.size());
	 * }
	 * 
	 * @Test public void testSearchDateRange() { final Session session =
	 * InitDatabase.getSession(); final FullTextSession fullTextSession = Search
	 * .createFullTextSession(session);
	 * 
	 * final Calendar rightNow = Calendar.getInstance(); rightNow.set(2008, 7,
	 * 25); final Date startDate = rightNow.getTime(); rightNow.set(2008, 8, 1);
	 ...
	 * DateTools.Resolution.DAY)), true); final org.hibernate.Query hibQuery =
	 * fullTextSession .createFullTextQuery(rq, Sample.class); final
	 * List<Sample> results = hibQuery.list(); assertEquals(5, results.size());
	 * }
	 * 
	 * @Test public void testPublicOrUserSamples() { final User testUser = new
	 * User(); testUser.setId(2); testUser.setName("matt");
	 ...
	 * TermQuery termQuery2 = new TermQuery(new Term("user_Name", //
	 * testUser.getName())); final TermQuery termQuery2 = new TermQuery(new
	 * Term( "user_emailAddress", testUser.getEmailAddress()));
	 ...
	 * 
	 * @Test public void testSampleFilteringByUserWithDates() { final User
	 * testUser = new User(); testUser.setId(2); testUser.setName("matt");
	 * 
	 * final Session session = InitDatabase.getSession(); final FullTextSession
	 * fullTextSession = Search .createFullTextSession(session);
	 * 
	 ...
	 * TermQuery termQuery = new TermQuery(new Term("publicData",
	 * Boolean.TRUE.toString())); privacyQuery.add(termQuery,
	 * BooleanClause.Occur.SHOULD); // Is this the current user? final TermQuery
	 * termQuery2 = new TermQuery(new Term("user_Name", testUser.getName()));
	 * privacyQuery.add(termQuery2, BooleanClause.Occur.SHOULD);
	 * 
	 * BooleanQuery fullQuery = new BooleanQuery();
	 * 
	 ...
	 * are met fullQuery.add(privacyQuery, BooleanClause.Occur.MUST);
	 * 
	 * // Date Search final Calendar rightNow = Calendar.getInstance();
	 * rightNow.set(2008, 7, 25); final Date startDate = rightNow.getTime();
	 * rightNow.set(2008, 8, 1); final Date endDate = rightNow.getTime();
	 * 
	 * RangeQuery rq = new RangeQuery(new Term("collectionDate", DateTools
	 * .dateToString(startDate, DateTools.Resolution.DAY)), new Term(
	 * "collectionDate", DateTools.dateToString(endDate,
	 ...
	 * 
	 * fullQuery.add(rq, BooleanClause.Occur.MUST); final org.hibernate.Query
	 * hibQuery = fullTextSession .createFullTextQuery(fullQuery, Sample.class);
	 ...
	 * 
	 * @Test public void testOxideSearch(){ final SearchSample searchSamp = new
	 * SearchSample(); Oxide tempOxide = new Oxide();
	 ...
	 * final RangeFilter rangeFilter = new
	 * RangeFilter("subsample_chemicalAnalysis_oxides_amount",
	 * NumberUtils.float2sortableStr(o.getLowerBound()),
	 * NumberUtils.float2sortableStr(o.getUpperBound()),true, true); final
	 * TermQuery oxideQuery = new TermQuery(new
	 * Term("subsample_chemicalAnalysis_oxides_oxide_species", o.getSpecies()));
	 ...
	 * columnName.equals("subsample_chemicalAnalysis_elements")){ if (((Set)
	 * methodResult).size() > 0) { final BooleanQuery setQuery = new
	 * BooleanQuery(); for (SearchElement o : (Set<SearchElement>) methodResult)
	 * { final RangeFilter rangeFilter = new
	 * RangeFilter("subsample_chemicalAnalysis_elements_amount",
	 * NumberUtils.float2sortableStr(o.getLowerBound()),
	 * NumberUtils.float2sortableStr(o.getUpperBound()),true, true); final
	 * TermQuery elementQuery = new TermQuery(new
	 * Term("subsample_chemicalAnalysis_elements_element_symbol",
	 * o.getElementSymbol())); final FilteredQuery filter = new
	 * FilteredQuery(elementQuery, rangeFilter); setQuery.add(filter,
	...
	public void testElementRangeSearch() {
		...
		for (SearchProperty i : enums) {
			...
			if (methodResult == null) {
				...
				if (columnName.equals("subsample_chemicalAnalysis_elements")) {
					if (((Set) methodResult).size() > 0) {
						final BooleanQuery setQuery = new BooleanQuery();
						for (SearchElement o : (Set<SearchElement>) methodResult) {
							final RangeFilter rangeFilterOnMin = new RangeFilter(
...
/*							final RangeFilter rangeFilterOnMin = new RangeFilter("subsample_chemicalAnalysis_oxides_minAmount", NumberUtils.float2sortableStr(-99999f), NumberUtils.float2sortableStr(o
							...
							final RangeFilter rangeFilterOnMin = new RangeFilter("subsample_chemicalAnalysis_oxides_amount", NumberUtils.float2sortableStr(-10f), NumberUtils.float2sortableStr(10f),true, true);							
							...
							setQuery.add(filterOnMinQuery, BooleanClause.Occur.SHOULD);
						}
						// require that one of these results be found in the
						// full query
						fullQuery.add(setQuery, BooleanClause.Occur.MUST);
